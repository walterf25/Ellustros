...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	Root.LoadLayout("StandingRecommendations")
	...
	lottieview.setAnimationFromJson("standing.json")
	
	CalculateScaleXY
	
	Dim j As JSONParser
	j.Initialize(File.ReadString(File.DirAssets, "standing.json"))
	
	Dim m As Map = j.NextObject
	Log("m: " & m)
	Dim Layers As List = m.Get("layers")
	LayersInfo = ParseJson2(Layers)
...
'''			parentLayer = layer
'''			Log("parent x: " & parentLayer.posX)
'''			Log("parent y: " & parentLayer.posY)
'''			Log("parent achor x: " & parentLayer.anchorX)
'''			Log("parent anchor y: " & parentLayer.anchorY)
'''			Log("parent scale x: " & parentLayer.ScaleX)
'''			Log("parent scale y: " & parentLayer.ScaleY)
'''			Exit
'''		End If
	'''	Next

	parentLayer.Initialize
	parentLayer.layerName = "N"
	parentLayer.posX = 219
	parentLayer.posY = 231
	parentLayer.anchorX = 0
	parentLayer.anchorY = 0
	parentLayer.ScaleX = 100
	parentLayer.ScaleY = 100
	...
	For Each layerdata As XYPosition In LayersInfo
		...
		If layerdata.layerName <> "N" Then
			...
			Select childLayer.layerName
				Case "2"
					lbl.Text = NumberFormat(eyeheight, 0, 1)
					'''lbl.Text = "8"
				Case "1"
					lbl.Text = NumberFormat(elbow, 0, 1)
					'''lbl.Text = "7"
'''				Case "6"
...
'''					'''lbl.Text = "3"
'''				Case "2"
'''					lbl.Text = NumberFormat(Common.BackRestAngle, 0, 0)
'''					'''lbl.Text = "2"
'''				Case "1"
'''					lbl.Text = Common.SeatTilt
'''					'''lbl.Text = "1"
			End Select
			'''[370.427,355.787,0]
...
Private Sub ParseJson2(layers As List)As List
	...
	For Each m As Map In layers
		...
		If layername = "N" Or layername = "2" _
		Or layername = "1" And Not(ks.ContainsKey("y")) Then
...
Sub lottie_AnimationEnded()
	...
	For Each v As B4XView In Root.GetAllViewsRecursive
		...
		Log("view tag: " & v.Tag)
		Log("type: " & GetType(v))
		If v Is Label And v.Tag <> Null And (v.Tag = "1" Or v.Tag = "2") Then
			Dim lbl As Label = v
			lbl.Visible = True
		End If
	Next
