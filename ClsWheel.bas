...
'	Public ItemNb = 7 As Int
	...
	Private lineWidth = 2dip							As Int
	Private colBackGround  = Colors.Blue	As Int
	Private colShadow  = Colors.white			As Int
	Private colWindowLine  = Colors.Red		As Int
	Private colWindow  = Colors.Transparent As Int '''Colors.ARGB(0, 255, 255, 255)	As Int   '''96, 255, 0, 0
	Private colScreen = Colors.Transparent As Int'''Colors.ARGB(196, 0, 0, 0) As Int
	...
	Private UnitsSet As Boolean = False
...
Public Sub Initialize(CallbackModule As Object, Parent As Object, cTitle As String, cWheelNb As Int, cWheelContent() As List, cFontSize As Float, cWheelType As Int, cContinusScrolling As Boolean, colwidthoffset As Int)
'''	Log("initializing clswheel")
	Dim i, j As Int
	...
	myfont = Typeface.LoadFromAssets("Cera Pro Medium.otf")
	...
	Do Until pnlMainWidth <= 100%x And pnlMainHeight <= 100%y
		...
		For i = 0 To WheelNb - 1
			...
			Next 
			ColWidths(i) = ColWidths(i) + colwidthoffset
		...
		End If	
'''		Log($"pnlMainWidth = ${pnlMainWidth}"$)
'''		Log($"pnlMainHeight = ${pnlMainHeight}"$)
	Loop

	Left = 50%x - pnlMainWidth / 2
	Top = 20dip'''50%y - pnlMainHeight / 2
	...
	pnlMain.Color = Colors.white'''Colors.Black
	...
	pnlMain.AddView(btnOK, pnlMain.Width/2 - 300dip/2, pnlMain.Height - btnHeight - btnSpace, 300dip, 60dip)
	'''btnOK.SetBackgroundImage(LoadBitmap(File.DirAssets, "btnSave.png"))
	btnOK.Typeface = Common.myfont.ToNativeFont
	btnOK.TextColor = xui.Color_White
	btnOK.Text = "Save"
	btnOK.TextSize = 16
	'''btnOK.As(B4XView).Background = Common.LightBlueColor
	Common.LightBlueColor(btnOK)
...
'''			Log("for j = o to WheelContent(i).size + ItemNb " & j)
			Dim lbl As Label
			lbl.Initialize("")
			...
			lbl.TextColor = Colors.ARGB(255, 28, 30, 36) ''rgba(28, 30, 36, 1)
...
'''	Log("exiting initializing function")
End Sub
...
Public Sub setUnits(b As Boolean)
	UnitsSet = b
End Sub

Public Sub getUnits As Boolean
	Return UnitsSet
End Sub
...
Private Sub InitDefaultValues(DefaulValue As String)
	...
	Case DATE
		...
		End If
		Log("datetime.getyear(Ticks) " & DateTime.GetYear(Ticks))
		Log("cFirstYear: " & cFirstYear)
		Log("WheelContentNb " & WheelContentNb(2))
			ScrollPos(2) = lblHeight * (DateTime.GetYear(Ticks) -  cFirstYear + WheelContentNb(2)) '''cFirstYear +
		ScrollPos(0) = lblHeight * (DateTime.GetMonth(Ticks) - 1 + WheelContentNb(0))
		i = DateTime.GetMonth(DateTime.Now) - 1
		CurrentDayIndex = DateTime.GetDayOfMonth(Ticks) - 1
		If WheelContentNb(1) <> DaysPerMonth(i) Then
			UpdateDaysWheel(DaysPerMonth(i))
		End If
'		CurrentDayIndex = DateTime.GetDayOfMonth(Ticks) - 1
		ScrollPos(1) = lblHeight * (CurrentDayIndex + WheelContentNb(1))
	Case TIME_HM
...
Private Sub InitializeDate
	...
	WheelContentNb(2) = cNbYears
	For j = 0 To WheelContinus
		For i = 0 To cNbYears - 1
			WheelContent(2).Add(cFirstYear + i)
		Next
	Next
	
	'''Log("WheelContent(2) " & WheelContent(2))
	...
	For j = 0 To WheelContinus
		For i = 1 To 12
			...
			txt = txt.Replace(".", "")
'''			Log("text: " & txt)
			WheelContent(0).Add(txt)
		Next
	Next
'''	Log("WheelContent(0) " & WheelContent(0))
	DateTime.DateFormat = DateFormat
	...
	For j = 0 To WheelContinus
		For i = 1 To 31
			WheelContent(1).Add(i)
		Next
	Next
	
'''	Log("WheelContent(1) " & WheelContent(1))
...
If getUnits Then
	Dim y As Int
	Dim x As Int
	x = pnlMiddle.Width * 0.5 - 40dip
	y = 0.5 * pnlMiddle.Height + 8dip

	cvsMiddle.DrawText("ft", x, y, Typeface.DEFAULT, 18, Colors.LightGray, "LEFT")
	cvsMiddle.DrawText("in", pnlMiddle.Width - 40dip, y, Typeface.DEFAULT, 18, Colors.LightGray, "LEFT")
	pnlMiddle.Invalidate
End If
...
Private Sub ScrollFinish(S As ScrollView, Pos As Int)
	...
	If (WheelType = DATE Or WheelType = DATE_TIME) And (S = scvWheel(0) Or S = scvWheel(2)) Then
		Year = Floor(scvWheel(2).ScrollPosition / lblHeight + 0.5) - WheelContentNb(2) + cFirstYear
		IndexMonth = Floor(scvWheel(0).ScrollPosition / lblHeight + 0.5) - WheelContentNb(0)
...
Private Sub UpdateDaysWheel(NewNbDays As Int)
	...
	WheelContentNb(1) = NewNbDays
	WheelContent(1).Clear
	For j = 0 To WheelContinus
	...
	scvWheel(1).Panel.RemoveAllViews
	For j = 0 To WheelContent(1).Size + ItemNb
		Dim lbl As Label
		lbl.Initialize("")
		scvWheel(1).Panel.AddView(lbl, 0, j * lblHeight, ColWidths(1), lblHeight)
		lbl.Gravity = Gravity.CENTER_HORIZONTAL + Gravity.CENTER_VERTICAL
		lbl.Color = Colors.White
		lbl.Typeface = myfont
		lbl.TextColor = Colors.ARGB(255, 28, 30, 36)
		lbl.TextSize = FontSize
		If j >= ItemNb_2 And j <= WheelContent(1).Size + ItemNb_2 - 1 Then
			lbl.Text = WheelContent(1).get(j - ItemNb_2)
		Else
			lbl.Text = ""
		End If
	Next
	
	scvWheel(1).Panel.Height = (WheelContent(1).Size + ItemNb_2 - 1)* lblHeight
	ScrollPos(1) = lblHeight * (CurrentDayIndex + WheelContentNb(1))
	Sleep(0)
	objWheel(1).RunMethod3("scrollTo", 0, "java.lang.int", ScrollPos(1), "java.lang.int")
End Sub
...
Private Sub UpdateWheel(Index As Int)
	...
	scvWheel(Index).Panel.RemoveAllViews
	For j = 0 To WheelContent(Index).Size + ItemNb
		...
		End If
	Next
	scvWheel(Index).Panel.Height = (WheelContent(Index).Size + ItemNb - 1)* lblHeight
End Sub
